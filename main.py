...
def main():
    ...
    if args.algo == 'a2c':
        agent1 = algo.A2C_ACKTR(actor_critic, args.value_loss_coef,
                               args.entropy_coef, lr=args.lr,
                               eps=args.eps, alpha=args.alpha,
                               max_grad_norm=args.max_grad_norm)
        agent2 = algo.A2C_ACKTR(actor_critic, args.value_loss_coef,
                               args.entropy_coef, lr=args.lr,
                               eps=args.eps, alpha=args.alpha,
                               max_grad_norm=args.max_grad_norm)
    elif args.algo == 'ppo':
        agent1 = algo.PPO(actor_critic, args.clip_param, args.ppo_epoch, args.num_mini_batch,
                         args.value_loss_coef, args.entropy_coef, lr=args.lr,
                               eps=args.eps,
                               max_grad_norm=args.max_grad_norm)
        agent2 = algo.PPO(actor_critic, args.clip_param, args.ppo_epoch, args.num_mini_batch,
                         args.value_loss_coef, args.entropy_coef, lr=args.lr,
                               eps=args.eps,
                               max_grad_norm=args.max_grad_norm)
    ...
    # obs_shape = envs.observation_space.shape
    agent1_rollouts = RolloutStorage(args.num_steps, args.num_processes, obs_shape, action_space, actor_critic.state_size)
    ...
    agent2_rollouts = RolloutStorage(args.num_steps, args.num_processes, obs_shape, action_space, actor_critic.state_size)
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            # Sample actions
            with torch.no_grad():
                value1, value2, action, action_log_prob, states1, states2 = actor_critic.act(
            ...
            obs1, reward1, done1, info1 = envs.agent1_step(cpu_actions)
            obs2, reward2, done2, info2 = envs.agent2_step(cpu_actions)
            ...
            update_current_obs(agent1_obs, agent1_current_obs, obs_shape, args.num_stack)
            agent1_rollouts.insert(agent1_current_obs, states1, action, action_log_prob, value1, reward, masks)

            update_current_obs(agent2_obs, agent2_current_obs, obs_shape, args.num_stack)
            agent2_rollouts.insert(agent2_current_obs, states2, action, action_log_prob, value2, reward, masks)
        ...
        with torch.no_grad():
            next_value1 = actor_critic.get_value(agent1_rollouts.observations[-1],
                                                agent1_rollouts.states[-1],
                                                agent1_rollouts.masks[-1]).detach()
            next_value2 = actor_critic.get_value(agent2_rollouts.observations[-1],
                                                agent2_rollouts.states[-1],
                                                agent2_rollouts.masks[-1]).detach()

        
        
        a1_obs = agent1_rollouts.observations[-1]
        a1_states = agent1_rollouts.states[-1]
        a1_masks = agent1_rollouts.masks[-1]
        a2_obs = agent2_rollouts.observations[-1]
        a2_states = agent2_rollouts.states[-1]
        a2_masks = agent2_rollouts.masks[-1]
        ...
        value_loss1, action_loss1, dist_entropy1 = agent1.update(agent1_rollouts, agent2_rollouts)
        value_loss2, action_loss2, dist_entropy2 = agent2.update(agent2_rollouts, agent1_rollouts)
        ...
        if j % args.save_interval == 0:
            ...
            torch.save((actor_critic.state_dict(), results_dict), os.path.join(
                model_dir, name + 'cifar_model_ppo_ex4.pt'))
        ...
        if j % args.log_interval == 0:
            ...
            total_reward1 = agent1_eval_episode(eval_env, actor_critic, args)
            total_reward2 = agent2_eval_episode(eval_env, actor_critic, args)
    ...
    plt.savefig("rewards_multi_ex4.png")
    plt.close()

    
    plt.plot(range(len(plot_policy_loss)), plot_policy_loss)
    plt.savefig("policyloss_multi_ex4.png")
    plt.close()

    
    plt.plot(range(len(plot_value_loss)), plot_value_loss)
    plt.savefig("valueloss_multi_ex4.png")
    plt.close()
