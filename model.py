...
class Policy(nn.Module):
    ...
    def act(self, a1_inputs, a1_states, a1_masks, a2_inputs, a2_states, a2_masks, deterministic=False):
        a1_value, a1_actor_features, a1_states = self.base(a1_inputs, a1_states, a1_masks)
        a2_value, a2_actor_features, a2_states = self.base(a2_inputs, a2_states, a2_masks)

        dist = self.dist(a1_actor_features, a2_actor_features)
        self.a1_actor_features = a1_actor_features
        self.a2_actor_features = a2_actor_features
        if deterministic:
            action = dist.mode()
        else:
            action = dist.sample()
        ...
        if self.dataset in IMG_ENVS:
            clf = self.clf(self.a1_actor_features, self.a2_actor_features)
            if deterministic:
                classif = clf.mode()
            else:
                classif = clf.sample()
        ...
        #states = torch.cat((a1_states, a2_states),0)
        #states = a1_states + a2_states

        return a1_value, a2_value, action, action_log_probs, a1_states, a2_states
    ...
    def evaluate_actions(self, a1_inputs, a1_states, a1_masks, a2_inputs, a2_states, a2_masks, action):
        a1_value, a1_actor_features, a1_states = self.base(a1_inputs, a1_states, a1_masks)
        a2_value, a2_actor_features, a2_states = self.base(a2_inputs, a2_states, a2_masks)
        ...
        if self.dataset in IMG_ENVS:
            clf = self.clf(a1_actor_features, a2_actor_features)
            action_log_probs = clf.log_probs(action[:, 1])
            action = action[:, 0]
        action_log_probs = dist.log_probs(action)
        ...
        #states = torch.cat((a1_states, a2_states),0)
        value = a1_value + a2_value
        return value, action_log_probs, dist_entropy, a1_states, a2_states
