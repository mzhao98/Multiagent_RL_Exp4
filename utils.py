...
def agent1_eval_episode(env, agent1, args):
    ...
    while not done1:
        ...
        a1_value, a2_value, action, action_log_probs, a1_states, a2_states = agent1.act(
        ...
        #      current_obs, states, FloatTensor([[0.0]]),
        #      deterministic=True)
        obs1, reward1, done1, _ = env.agent1_step(action.detach().cpu().numpy())
        #obs2, reward2, done2, _ = env.agent2_step(action.detach().cpu().numpy())
        #print("REWARD", reward)
...
def agent2_eval_episode(env, agent2, args):
    ...
    while not done2:
        # state = torch.from_numpy(obs).float().cuda()
        update_current_obs(obs, current_obs, obs_shape, args.num_stack)
        a1_value, a2_value, action, action_log_probs, a1_states, a2_states = agent2.act(
             current_obs, states, FloatTensor([[0.0]]), current_obs, states, FloatTensor([[0.0]]),
             deterministic=True)
        # value2, action2, action_log_probs2, states2 = agent2.act(
        #      current_obs, states, FloatTensor([[0.0]]),
        #      deterministic=True)
